...
SystemJS.register([], function (_export, _context) {
  ...
  return {
    ...
    execute: function () {
      ...
      Balloon = function Balloon(targetEl) {
      ...
      Balloon.prototype = {
        ...
        speak: function speak(complete, text, hold, callback) {
          this._hidden = false;
          this.show();
          var c = this._content;
          // set height to auto
        ...
        ask: function ask(complete, text, choiceTexts, callback) {
          var choices = [];
          for (var i = 0; i < choiceTexts.length; i++) {
            var d = $('<a class="clippy-choice"></a>').text(choiceTexts[i]);
            choices.push(d);
          }
        ...
        _sayWords: function _sayWords(text, choices, hold, complete, callback, isQuestion) {
          this._active = true;
          this._hold = hold;
